...
#'@export
meMultiplex_infomapCommunities <- function(cor_layer, supplementary_layer, physical_nodes=FALSE, layer=1, folder="./meNet/", file_basename="meNet_infomap", infomap_call="infomap",
  ...
  meMultiplex_generic(cor_layer, supplementary_layer, meNet_file, cor_weighted, supp_weighted, cor_normalization_fun, supp_normalization_fun, output_type="infomap", inter_cor_supp, inter_supp_cor)
  ...
  if(physical_nodes){
    communities <- infomapCommunities_fromFiles(meNet_file, CLU_file, physical_nodes)
  }else{
    communities <- infomapCommunities_fromFiles(meNet_file, statesCLU_file, physical_nodes, layer)
  }
  if(delete_files){
    is_deleted <- FALSE
    #
